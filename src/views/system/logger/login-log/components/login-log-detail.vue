  ...
  <el-dialog v-model="dialogVisible" title="详情" width="800">
    <el-descriptions :column="1" border v-if="detailData"
      ...
      <el-descriptions-item label="操作类型">
        <el-tag v-if="detailData.logType === 100" type="primary"
          >账号登录</el-tag
        >
        <el-tag v-if="detailData.logType === 101" type="info">社交登录</el-tag>
        <el-tag v-if="detailData.logType === 103" type="warning"
          >短信登录</el-tag
        >
        <el-tag v-if="detailData.logType === 200" type="primary"
          >主动登出</el-tag
        >
        <el-tag v-if="detailData.logType === 202" type="danger"
          >强制登出</el-tag
        >
      </el-descriptions-item>
      ...
      <el-descriptions-item label="登陆结果">
        ...
        <el-tag v-if="detailData.result === 10" type="info"
          >账号或密码不正确</el-tag
        >
        <el-tag v-if="detailData.result === 20" type="warning"
          >用户被禁用</el-tag
        >
        <el-tag v-if="detailData.result === 30" type="primary"
          >验证码不存在</el-tag
        >
        <el-tag v-if="detailData.result === 31" type="danger"
          >验证码不正确</el-tag
        >
